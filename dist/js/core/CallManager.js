...
export class CallManager extends EventEmitter {
    ...
    async provisionPhoneNumber(country = 'DE', type = 'local') {
        try {
    ...
    async initiateOutboundCall(phoneNumber, session, fromNumber) {
        try {
            if (!this.twilioClient) {
                throw new Error('Twilio client not initialized');
            }
    ...
    async handleIncomingCall(phoneNumber, callSid, metadata = {}) {
    ...
    async createSIPCall(phoneNumber, session, sipConfig) {
        try {
    ...
    async endCall(sessionId) {
        ...
        try {
            ...
            if (session.metadata.sipInviter) {
                const inviter = session.metadata.sipInviter;
                await inviter.bye();
            }
            // Update session
            session.status = 'completed';
    ...
    async transferCall(sessionId, targetPhoneNumber, options = {}) {
        const session = this.activeCalls.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async handleTwilioWebhook(event, data) {
        switch (event) {
            case 'call-progress':
