...
import { APIResponse, VoiceAssistant } from '../../types';
...
router.post('/', auth, validateRequest(createAssistantSchema), async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
...
router.put('/:id', auth, validateRequest(updateAssistantSchema), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
...
router.post('/:id/duplicate', auth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name } = req.body;
    const userId = req.user!.id;
